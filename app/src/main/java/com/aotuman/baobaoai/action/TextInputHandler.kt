...
class TextInputHandler(private val service: AutoGLMService) {
    ...
    private val keyboardAgent by lazy { KeyboardAgent(service) }

    fun setDebugListener(listener: (Bitmap, String) -> Unit) {
        keyboardAgent.onDebugInfo = listener
        KeyboardScanner.setCandidateDebugListener(listener)
    }
